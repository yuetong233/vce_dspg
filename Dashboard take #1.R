...
ui <- fluidPage(
  ...
  div(
    ...
    # === LOGO ===
    conditionalPanel(
      condition = "input.tabs == '4-H Participation County Data' || input.tabs == '4-H Participation Trends'",
...
server <- function(input, output, session) {
  ...
  # === NEW: READ PEARS_BI VOLUNTEER AND PARTICIPANT DATA ===
  pears_bi_df <- read_excel("PEARS_BI Volunteer and Participant 2025.xlsx") %>%
    mutate(
  ...
  # === NEW: RENDER VCE VOLUNTEERS VS PARTICIPATION MAP ===
